...
public class SimplePermissionNotice {
    ...
    private SimplePermissionNotice(Context context, UiConfig.Builder builder, PermissionInfo... permissionInfos) {
        this(context, builder, -1, permissionInfos);
    }

    private SimplePermissionNotice(Context context, UiConfig.Builder builder, int itemDividerDrawableResourceId, PermissionInfo... permissionInfos) {
        ...
        for (PermissionInfo each : permissionInfos) {
            ...
            mPermissionSettings.put(permissionCode, new PermissionInfo(permissionCode, permissionTitle, permissionDesc, each.getPermissionIconResourceId(), each.isMandatoryPermission()));
        }
    ...
    public static void init(Context context, UiConfig.Builder builder, PermissionInfo... permissionInfos) {
        if (sInstance == null) {
            sInstance = new SimplePermissionNotice(context, builder, permissionInfos);
        }
    }

    public static void init(Context context, UiConfig.Builder builder, int itemDividerDrawableResourceId, PermissionInfo... permissionInfos) {
        if (sInstance == null) {
            sInstance = new SimplePermissionNotice(context, builder, itemDividerDrawableResourceId, permissionInfos);
        }
    }
    ...
    private ArrayList<ArrayList<PermissionInfo>> getPermissions(@DangerousPermissions String... permissionCodes) {
        ArrayList<PermissionInfo> mandatoryPermissions = new ArrayList<>();
        ArrayList<PermissionInfo> optionalPermissions = new ArrayList<>();
    ...
    public void showDialog(Activity activity, SimplePermissionNoticeCallback callback, @DangerousPermissions String... permissionCodes) {
        if (!checkSdkLevel(callback)) {
            return;
        }
    ...
    public void showActivity(Activity activity, SimplePermissionNoticeCallback callback, @DangerousPermissions String... permissionCodes) {
        if (!checkSdkLevel(callback)) {
            return;
        }
    ...
    public void showActivity(Activity activity, int activityLayoutResourceId, SimplePermissionNoticeCallback callback) {
        ...
        ArrayList<ArrayList<PermissionInfo>> permissions = getPermissions();
        launchActivity(activity, permissions.get(0), permissions.get(1), activityLayoutResourceId, callback);
    }
